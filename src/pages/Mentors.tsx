...
export function Mentors() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-white">
      ...
      <div className="max-w-5xl mx-auto px-6 space-y-2 py-6 relative">
        <div className="mb-6 text-sm text-gray-700 space-y-2 max-w-3xl">
          <p className="text-xl font-semibold leading-relaxed">
            You're seeing mentors who truly fit your learning journey.
          </p>
          <p className="text-sm leading-relaxed">
            This list is carefully curated using the courses you're enrolled in,
            the domain you're exploring, and the skills you want to build. We
            also consider your interests and progress, so the guidance you get
            feels relevant — not generic. Each mentor here understands where you
            are right now and what comes next.
          </p>
        ...
        <div>
          <div className="flex items-center justify-center gap-0  w-[100%]">
            ...
            <div
              className="domain-scroll flex gap-2 overflow-x-auto scrollbar-hide cursor-grab active:cursor-grabbing"
              style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
            >
              {domains.map((domain, index) => (
                <span
                  key={index}
                  className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm whitespace-nowrap flex-shrink-0"
                >
                  {domain}
                </span>
              ))}
        ...
        <div className="grid grid-cols-3 gap-4 pt-4">
          {topMentors.map((mentor, index) => (
            <div className="relative">
              ...
              <div
                key={mentor.id}
                className="bg-white rounded-3xl border border-gray-200  hover:shadow-md transition-shadow relative overflow-hidden"
              >
                <div className="bg-white h-20 relative">
                  <div className="absolute top-0 left-0 h-[70px]">
                    <img
                      src={index % 2 === 0 ? bgpattern : bgpatternnew2}
                      alt="image"
        ...
        <div className="grid grid-cols-3 gap-4 pb-8">
          {mentors.map((mentor, index) => (
            <div
              key={mentor.id}
              className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow relative"
            >
              <div className="bg-white h-20 relative">
